...
export default class LargeUploader extends React.Component {
  ...
  handleRename = (e) => {
    console.log('handleRename', e, e.target.value)
    this.setState({
  ...
  renderName(item) {
    ...
    if (canIRename) {
      return (
        <Fragment>
          <input type="text" className="file-name" defaultValue={file_name} onChange={this.handleRename.bind(this)} />
          <span className="file-name" onClick={this.renameFile(id)}>确认</span>
        </Fragment>
      )
    } else {
      return (
        <Fragment>
          ...
          <span className="file-name" onClick={this.resetName(id, name)}>重命名</span>
        </Fragment>
      )
    }
  }
  ...
  renderFileList() {
    ...
    return fileList.map((item) => {
      const { id, percentage, uploadStatus } = item;
